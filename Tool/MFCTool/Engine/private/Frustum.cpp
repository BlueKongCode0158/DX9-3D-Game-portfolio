...
void CFrustum::Update_Frustum()
{
	...
	_matrix	ProjectionMatrix = pPipeLine->Get_Transform(D3DTS_PROJECTION);
	_matrix ViewMatrix = pPipeLine->Get_Transform(D3DTS_VIEW);

	D3DXMatrixInverse(&ProjectionMatrix, nullptr, &ProjectionMatrix);
	D3DXMatrixInverse(&ViewMatrix, nullptr, &ViewMatrix);

	for (_uint i = 0; i < 8; i++)
	{
		D3DXVec3TransformCoord(&m_vWorldPoint[i], &m_vOriginal_Point[i], &ProjectionMatrix);
		D3DXVec3TransformCoord(&m_vWorldPoint[i], &m_vOriginal_Point[i], &ViewMatrix);
	}
...
_bool CFrustum::isIn(_float3 vWorldPos, _float fRange)
{
	for (_uint i = 0; i < 6; i++)
	{
...
CFrustum * CFrustum::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	...
	{
		MSGBOX("Failed to Creating CFrustum_Prototype");
