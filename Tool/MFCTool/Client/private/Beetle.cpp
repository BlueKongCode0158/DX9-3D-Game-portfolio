#include "stdafx.h"
#include "..\public\Beetle.h"
#include "GameInstacne.h"
#include "MonsterState.h"
#include "Normal_Monster_Hp.h"

CBeetle::CBeetle(LPDIRECT3DDEVICE9 pGraphic_Device)
	:CMonster(pGraphic_Device)
{
}

CBeetle::CBeetle(const CBeetle & rhs)
	: CMonster(rhs)
	, m_pHpBar(reinterpret_cast<CNormal_Monster_Hp*>(rhs.m_pHpBar->Clone()))
{
}

HRESULT CBeetle::NativeConstruct_Prototype()
{
...
HRESULT CBeetle::NativeConstruct(void * pArg)
{
	...
	if (nullptr == pArg)
	{
		m_pTransformCom->Set_WorldMatrixRow(CTransform::STATE::STATE_POSITION, _float3(69.f, 0.f, 62.f));
	}
	else
	{
		m_pTransformCom->Set_WorldMatrixRow(CTransform::STATE::STATE_POSITION, *reinterpret_cast<_float3*>(pArg));
	}
	m_pMonsterState = CMonsterState::Create(m_pGraphicDevice, CMonsterState::MONSTERTYPE::BEETLE, this);
	...
	tHpDesc.vIniPos = _float3(0.f, 4.f, 0.f);
...
_int CBeetle::Tick(double TimeDelta)
{
	...
	m_pTransformCom->Set_WorldMatrixRow(CTransform::STATE::STATE_POSITION, vPosition + _float3(0.f, 1.8f, 0.f));
...
_int CBeetle::Late_Tick(double TimeDelta)
{
...
HRESULT CBeetle::Add_Component()
{
	if (FAILED(__super::Add_Component(LEVEL_GAMEPLAY0, TEXT("Prototype_Beetle_Mesh"), TEXT("Com_Mesh"), reinterpret_cast<CComponent**>(&m_pMeshCom))))
	{
		return E_FAIL;
	}
	...
	ColliderDesc.pParentMatrix = m_pTransformCom->Get_WorldMatrix();
	ColliderDesc.vScale = _float3(500.f, 500.f, 500.f);
	ColliderDesc.vIniPos = _float3(0.f, 0.f, 0.f);
...
HRESULT CBeetle::SetUp_ConstantTable()
{
...
CBeetle * CBeetle::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CBeetle* pInstance = new CBeetle(pGraphic_Device);
	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		Safe_Release(pInstance);
		MSGBOX("Failed to Creating CBeetle_Prototype");
	}
	return pInstance;
}

CGameObject * CBeetle::Clone(void * pArg)
{
	CBeetle* pInstance = new CBeetle(*this);
	if (FAILED(pInstance->NativeConstruct(pArg)))
	{
		Safe_Release(pInstance);
		MSGBOX("Failed to Creating CBeetle_Clone");
	}
	return pInstance;
}
